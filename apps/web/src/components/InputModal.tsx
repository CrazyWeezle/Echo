  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md rounded-xl border border-neutral-800 bg-neutral-900 p-4 shadow-2xl">
        ...
        {textarea ? (
          <textarea ref={ref as any} rows={4} className="w-full p-2.5 rounded-md bg-neutral-950 text-neutral-100 placeholder-neutral-500 border border-neutral-800 focus:outline-none focus:ring-2 focus:ring-emerald-600/60" placeholder={placeholder} value={val} onChange={(e)=>setVal(e.target.value)} spellCheck={true} autoCorrect="on" autoCapitalize="sentences" />
        ) : (
          <input ref={ref as any} className="w-full p-2.5 rounded-md bg-neutral-950 text-neutral-100 placeholder-neutral-500 border border-neutral-800 focus:outline-none focus:ring-2 focus:ring-emerald-600/60" placeholder={placeholder} value={val} onChange={(e)=>setVal(e.target.value)} onKeyDown={(e)=>{ if(e.key==='Enter' && !e.shiftKey){ e.preventDefault(); onSubmit(val.trim()); } if(e.key==='Escape'){ e.preventDefault(); onCancel(); } }} />
        )}
